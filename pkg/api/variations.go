...
func (c *Client) Variations(ctx context.Context, projectKey, featureKey string) ([]Variation, error) {
	...
	path := fmt.Sprintf("/projects/%s/features/%s/variations", url.PathEscape(projectKey), url.PathEscape(featureKey))
...
func (c *Client) Variation(ctx context.Context, projectKey, featureKey, variationKey string) (*Variation, error) {
	...
	path := fmt.Sprintf("/projects/%s/features/%s/variations/%s", url.PathEscape(projectKey), url.PathEscape(featureKey), url.PathEscape(variationKey))
